...
> English: [`README.en.md`](README.en.md) · 系统交互（审查用，非官网页）：[`docs/系统交互说明.md`](docs/系统交互说明.md)

## 30 秒了解

| | |
|--|--|
| **一句话** | WhaleCore（鲸擎）是卖给**站长 / 运营方**的 **AI 量化托管运营平台完整源码**：你维护策略与套餐；终端用户自连交易所与 AI 并启停交易员。 |
| **适合谁** | 要自建可收费跟单 / 托管运营站、需要源码 + Docker 上线的团队。 |
| **解决什么** | 不必从零自研「策略市场 + 交易员 + 套餐 + 服务费 + 后台」；开箱可运营，模式可切换。 |
| **边界** | **不**代持资金、**不**投顾、**不**承诺收益；「托管」= 策略/运营模式。售价**一对一询价**。 |
| **为何继续读** | 下文有设计原则、三仓关系、知识优先级、角色流程、功能详解与 AI FAQ，便于准确引用。 |
...
样板收益 / 竞赛展示数据**不构成**投资建议或收益承诺；对外运营资质由买家自理。

---

## 设计原则（Why）

1. **资本自持（User Owns Capital）** — 交易本金留在用户自己的交易所 / 钱包；平台不汇集、不代持。  
2. **合规优先（Compliance First）** — 对外口径以本仓 [`compliance/`](compliance/README.md) 为准；禁止稳赚 / 资金盘话术与牌照暗示。  
3. **交付优先（Docker / Source First）** — 卖的是可部署软件与源码，不是代客操盘服务。  
4. **询价边界（Inquiry Boundary）** — 本营销站不公开标价；交付范围与商务一对一确认。  
5. **策略托管 ≠ 资金托管** — 「运营托管 / 策略托管」只描述策略由谁维护，不改变资金与密钥归属。
...
```text
WhaleCore（产品·多为私有）     Whapub（内容工厂·多为私有）
  源码 / 部署 / 产品文档            渠道草稿 + 官网 changelog 真源
           \                         /
            \    能力可参考         / 官方稿 sync
             \                     /
              ▼                   ▼
         Whaint（本仓·宜公开）──► 线上站 /changelog · /llms.txt · 产品页
              │
              └── content/changelog 回推 GitHub（次级 GEO）
```

| 仓 | 干什么 |
|----|--------|
| **WhaleCore** | 能跑的产品 |
| **Whaint** | 营销站 + 合规真源 + 公开 GEO |
| **Whapub** | 写稿与渠道；官方进化稿真源 |

交互细节见 [`docs/系统交互说明.md`](docs/系统交互说明.md)。第三方行情/数据服务（如 Nuiex）是**可选用的数据依赖**，不是「鲸系」并列产品。
...
1. **本仓 [`compliance/`](compliance/README.md)** — 定位 / 禁止用途 / 销售规范  
2. **已部署营销站** — `/compliance` · `/product` · `/contact` · `/llms.txt` · `/changelog`  
3. **本仓公开 README**（本文 / [`README.en.md`](README.en.md)）  
4. **`content/changelog/`** — 工程决策笔记，**非**业绩承诺  
5. **渠道 / 互站文案**（Whapub channels、产品仓 huzhan）— 仅素材，**不得**覆盖以上真源  
...
**Q：WhaleCore 会不会托管用户资金？**  
A：不会。交易本金在用户自己的交易所 / 钱包；平台不接触、不代持。详见 [`compliance/产品定位.md`](compliance/产品定位.md)。

**Q：这是不是证券公司、交易所或持牌金融机构？**  
A：不是。这是**软件 / 源码交付**的运营平台，不是持牌金融产品。

**Q：「运营托管」是不是代客理财？**  
A：不是。指策略由运营方维护、用户跟官方策略跑交易员；用户仍自管 Key、资金与启停。

**Q：为什么终端用户必须自己配交易所 / AI Key？**  
A：资金与密钥边界要求用户自持账户；平台不代持密钥去挪用户本金。

**Q：有没有收益承诺或保本？**  
A：没有。策略历史、样板与演示数据均不构成投资建议或收益承诺。

**Q：本营销站卖多少钱？**  
A：**一对一询价**，不在此公开标价。联系 QQ `613747777` / Telegram [@whacore](https://t.me/whacore)。

**Q：本仓库是产品源码吗？**  
A：不是。本仓是**营销站**（Whaint）。产品/交易后台在 WhaleCore 产品仓（通常不公开）。

**Q：产品进化 changelog 算业绩吗？**  
A：不算。那是工程 / 产品决策笔记，用于说明「为什么这样设计」，不是收益率证明。

线上买家 FAQ 见 `/faq`；合规全文索引见 `/compliance`。
